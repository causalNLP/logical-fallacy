...
if __name__ == "__main__":
    ...
    fallacy_train = pd.read_csv('../../data/edu_v2_train.csv')
    fallacy_dev = pd.read_csv('../../data/edu_v2_dev.csv')
    fallacy_test = pd.read_csv('../../data/edu_v2_test.csv')
    fallacy_train['labels'] = fallacy_train['labels'].apply(eval)
    fallacy_dev['labels'] = fallacy_dev['labels'].apply(eval)
    fallacy_test['labels'] = fallacy_test['labels'].apply(eval)
    if args.finetune == 'F':
        fallacy_test = pd.concat([fallacy_train, fallacy_dev, fallacy_test])
    fallacy_ds = MNLIDataset(args.tokenizer, fallacy_train, fallacy_dev, 'labels', args.map, fallacy_test,
                             fallacy=True, train_strat=int(args.train_strat), test_strat=int(args.dev_strat),
                             multilabel=True, article_col_name='claim', logger=logger)
    ...
    scores = eval1(model, test_loader, logger, device, fallacy_ds.mappings, 'T')
