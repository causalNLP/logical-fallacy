...
class MNLIDataset:
    ...
    def __init__(self, tokenizer_path, train_df, val_df, label_col_name, map='base', test_df=None, fallacy=False,
                 ...
                 train_strat=1, test_strat=1, multilabel=False, positive_class_weight=12,
                 article_col_name='source_article', logger=None):
        # strat is used in convert_to_mnli function
        self.logger = logger
        self.article_col_name = article_col_name
    ...
    def convert_to_mnli(self, df, undersample=False, undersampling_rate=0.02, strat=1):
        ...
        for i, row in df.iterrows():
            for label in self.mappings['Original Name']:
                entry = [row[self.article_col_name]]
                if self.map == 'base':
                    ...
                    entry.append("This text matches the following logical form: %s" % form)
                elif self.map == 'hierarchy':
                    # print(self.mappings[self.mappings['Original Name'] == label]['Masked Logical Form'])
                    # print(label)
                    form = replace_masked_tokens(list(self.mappings[self.mappings['Original Name'] == label]
                                                      ['Masked Logical Form'])[0])
                    parent_fallacy = list(self.mappings[self.mappings['Original Name'] == label]['Parent Fallacy'])[0]
                    if not isinstance(parent_fallacy, float):
                        parent_form = list(self.mappings[self.mappings['Original Name'] == parent_fallacy]
                                           ['Masked Logical Form'])[0]
                        entry.append("This text matches the following logical form: %s, which is a type of the "
                                     "general logical form %s." % (form, parent_form))
                    else:
                        entry.append("This text matches the following logical form: %s" % form)
...
def get_metrics(logits, labels, threshold=0.5, sig=True, tensors=True):
    ...
    precision = sklearn.metrics.precision_score(y_true=y_true, y_pred=y_pred, average='micro', zero_division=0)
    recall = sklearn.metrics.recall_score(y_true=y_true, y_pred=y_pred, average='micro', zero_division=0)
...
def map(preds, mapping, reduce):
    if reduce != 'T':
        return preds
    ans = []
    dict = {}
    for i, row in mapping.iterrows():
        if isinstance(row['Parent Fallacy'], float):
            ans.append(preds[i])
            dict[row['Original Name']] = i
        else:
            ans[dict[row['Parent Fallacy']]] += preds[i]
    return torch.tensor(ans)


def eval1(model, test_loader, logger, device, mapping, reduce):
    with torch.no_grad():
        all_preds = []
        all_labels = []
        for batch_idx, (pair_token_ids, mask_ids, seg_ids, y, weights) in enumerate(test_loader):
            ...
            if model == "random":
                ...
                _, prediction = model(pair_token_ids,
                                      ...
                                      attention_mask=mask_ids,
                                      labels=labels).values()
            all_preds.append(map(torch.log_softmax(prediction, dim=1).argmax(dim=1), mapping, reduce))
            all_labels.append(map(labels, mapping, reduce))
        all_preds = 1 - torch.stack(all_preds)
        all_labels = 1 - torch.stack(all_labels)
        all_preds[all_preds < 0] = 0
        all_labels[all_labels < 0] = 0
        logger.info("%s %s", all_preds.shape, all_labels.shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-nt", "--do_not_train", help="Set this to T if you do not wish to train the model",
                        default='F')
    parser.add_argument("-r", "--reduce", help="Set this to T if you wish to map subclasses to their parents",
                        default='F')
    ...
    scores = eval1(model, test_loader, logger, device, fallacy_ds.mappings, args.reduce)
